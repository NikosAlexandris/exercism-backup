...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    threshold_10_percent = 10 * threshold / 100  
    if  product < threshold * 90 / 100:
        return 'LOW'  # control rods must be removed to produce power

    elif threshold - threshold_10_percent < product < threshold + threshold_10_percent:
        return 'NORMAL'  # reactor is in optimum condition and control rods are in an ideal position
    
    else:
        return 'DANGER'  # reactor is going into meltdown, shut down immediately !